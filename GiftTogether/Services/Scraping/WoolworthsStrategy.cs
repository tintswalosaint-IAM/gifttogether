...
public class WoolworthsStrategy : StrategyBase
{
    public override bool Matches(string host) =>
        host.EndsWith("woolworths.co.za", StringComparison.OrdinalIgnoreCase);
    ...
    public override (string? Title, decimal? Price, string? ImageUrl) Extract(HtmlDocument doc)
    {
        var title = doc.DocumentNode.SelectSingleNode("//h1[contains(@class,'product-name')]")?.InnerText?.Trim()
            ?? CleanTitle(PageTitle(doc) ?? OgTitle(doc), " - Woolworths");

        var priceRaw = doc.DocumentNode.SelectSingleNode("//strong[contains(@class,'price')]")?.InnerText
            ?? MetaPrice(doc);
        var price = ParseZarPrice(priceRaw);

        var imageUrl = OgImage(doc)
            ?? doc.DocumentNode.SelectSingleNode("//img[contains(@class,'product-image')]")?.GetAttributeValue("src", null);
